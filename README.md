...
**① PixVerse アプリで自分で作って上げる（追加費用なし・おすすめ）**

アプリで生成した mp4 を、実写と同じようにリリースに添付するだけです。
既にお持ちのアプリ内クレジットで作れるので、**API 用のクレジットを買う必要がありません。**

ファイル名の先頭に `ai_` を付けるか、名前に `pixverse` を含めてください。
それだけで AI 生成として記録され、実写との成績比較ができます。

```
ai_紅葉散歩.mp4        → AI 生成として取り込まれる
PixVerse_0921.mp4     → 同上
IMG_1234.MOV          → 実写として取り込まれる
```

**② API で自動生成させる（API 用クレジットが必要）**
...
写真を `images/pending/` に入れておくと、在庫が減ったときに自動で動画化します。
ただし <https://platform.pixverse.ai/> の **API 側の残高**が必要で、
アプリで買ったクレジットは使えない可能性が高い点に注意してください。
