...
- [ ] **AI 動画を混ぜる** — いまは実写のみ（`LIVE_RATIO=1.0`）
  ...
  **手軽な方法（追加費用なし）**: PixVerse アプリで自分で生成し、
  ファイル名を `ai_〜.mp4` にして実写と同じように添付するだけ。
  お持ちのアプリ内クレジットで作れるので、API 用クレジットは不要です。
  AI 生成として記録されるので、実写との成績比較もそのまま効きます。
  **設定変更も一切不要**で、今日から試せます。

  **API で自動生成させる場合**（在庫を自動補充したいとき）
